...
def search_downright(i, j):
    if i <= ROWS-4 and j <= COLS-4 and "".join(WORD_SEARCH[i+x, j+x] for x in range(4)) == "XMAS":
        return 1
    else:
        return 0
...
def p2_search(i, j):
    if ("".join(WORD_SEARCH[i+x, j+x] for x in range(-1, 2)) in ["MAS", "SAM"]
    and "".join(WORD_SEARCH[i-x, j+x] for x in range(-1, 2)) in ["MAS", "SAM"]):
        return 1
    else:
        return 0   
...
p2_search_points = [
    x for x in np.argwhere(WORD_SEARCH=="A")
    if 0 < x[0] < ROWS - 1
    and 0 < x[1] < COLS - 1
    ]
